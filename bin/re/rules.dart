...
String? A0(List<String> args, int index) {
...
final Array<String> Abbr = Array.from(['n', 'ne', 'se', 's', 'sw', 'nw']);
final Array<String> Full = Array.from(['north', 'northeast', 'southeast', 'south', 'southwest', 'northwest']);
...
void build(Unit u, List<String> args) {
  ...
  if ((args.isEmpty) && (u.type == Unit.Settlers)) {
    type = Unit.City;
  }
  if ((type < 0)) {
  ...
  if (RegExp('^[A-Za-z]').hasMatch(a1) && a2 == null) {
    a2 = a1;
  }
  final name = a2 ?? Unittypes[type].name;
  final cost = Unittypes[type].buildcost;
  if ((cost == 0)) {
  ...
  final u2 = Unit(id, name, type, p, u._hex);
  nextid(u2);
  addunit(u2);
...
void explore(Unit u, List<String> args) {
  ...
  switch (rand(5)) {
    ...
    default:
      ...
      r.special = (0);
      final hostile = (rand(6) == 0);
      final u2 = Unit(1, 'Ancient Battlemechs', Unit.Mechs, (hostile ? $players.last() : u.player), u._hex);
      nextid(u2);
      addunit(u2);
...
void fire(Unit u, List<String> args) {
  final range = Unittypes[u.type].range;
  if ((range == 0)) {
...
void friendly(Player p, List<String> args) {
  final p2 = findplayer(int.parse(args[0]));
  if ((p2 == null)) {
...
void give(Player p, List<String> args) {
  final p2 = findplayer(int.parse(args[1]));
  if (p2 == null) {
    p.event('Player not recognized');
    return;
  }
  final qty = min(max(int.parse(args[1]), 1), p.money);
  p.money = ((p.money - qty));
...
void move(Unit u, List<String> args) {
  ...
  final cost = Terrains[hex.terrain].movementcost;
  if ((u.movement < cost)) {
...
Unit? findtarget(Unit u) {
  ...
  final targets = u.hex().units.select((u2) {
...
void captureunit(Unit c, Hex h) {
  final a = h.units.shuffle();
  for (final u in a._elements) {
    if ((u.attack == 0)) {
      continue;
    }
    ...
    final c2 = Unit(1, c.name, c.type, u.player, h.h);
    nextid(c2);
    addunit(c2);
...
void income() {
  $units.each((u) {
    final income = (-Unittypes[u.type].upkeepcost);
    if ((income > 0)) {
...
void upkeep() {
  $players.each((p) {
    p.units.dup().each((u) {
      final upkeep = Unittypes[u.type].upkeepcost;
      if ((upkeep < 0)) {
        return;
      }
      if ((p.money >= upkeep)) {
