...
# Performs the EXPLORE order.
def explore(u, args) #sma
  if u.type != Unit::Settlers
    u.event("Only settlers may explore")
    return
  end
  r = u.hex!.units.detect {|u2| u2.ruin?}
  if !r
    u.event("Nothing found")
    return
  end
  if r.player != u.player && !(r.player.friendly.include? u.player) 
  ...
  when 0
    ...
    r.special = 0
    hostile = rand(6) == 0
    u2 = Unit.new(1, "Ancient Battlemechs", Unit::Mechs,
		  hostile ? $players.last : u.player,
		  u.hex)
    nextid(u2)
    addunit(u2)
    u.event(u.nameid, "discovered a new unit", u2.namepidtype)
    u.event("wait, that unit is out of control!") if hostile
